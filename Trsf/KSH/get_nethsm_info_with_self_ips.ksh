...
#
# Usage:
#   ./get_nethsm_info_with_self_ips.ksh
#   ./get_nethsm_info_with_self_ips.ksh my_hosts.inv
#   ./get_nethsm_info_with_self_ips.ksh my_hosts.inv output.tsv
#
# Environment:
...
HSM_SELF_COMMAND='bash -c "tmsh list net self one-line | grep -i HSM"'
...
HSM_SELF_OUTPUT="${TMP_BASE}.hsm_self"
...
printf 'hostname\tserver_ips\thsm_self_ips\tnethsm_partition\n' > "$OUTPUT_FILE"
...
do
    ...
    ###########################################################################
    # Obtain all HSM-facing BIG-IP self IP addresses
    ###########################################################################

    : > "$HSM_SELF_OUTPUT"
    : > "$SSH_ERROR"

    echo "+ ssh ${SSH_USER}@${HOST} '${HSM_SELF_COMMAND}'"

    sshpass -p "$PASSNET" \
        ssh -n $SSH_OPTIONS \
        "${SSH_USER}@${HOST}" \
        "$HSM_SELF_COMMAND" \
        > "$HSM_SELF_OUTPUT" 2> "$SSH_ERROR"

    HSM_SELF_STATUS=$?

    if [ "$HSM_SELF_STATUS" -eq 0 ]; then
        HSM_SELF_IPS=$(
            awk '
            {
                for (i = 1; i <= NF; i++) {
        ...
        if [ -n "$HSM_SELF_IPS" ]; then
            echo "  HSM self IPs     : $HSM_SELF_IPS"
        else
            echo "  WARNING: No HSM self IPs found."
        fi
    else
        echo "  ERROR: Failed to retrieve HSM self IPs (SSH exit $HSM_SELF_STATUS)."
