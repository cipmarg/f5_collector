...
function finish_cert() { if ((part=="" || part=="Common") && name!="" && epoch!="" && epoch+0>=cutoff) print name,expstr,serial,make_cu(created,updated),dn_value(subject,"O"),dn_value(subject,"CN"),dn_value(issuer,"CN"),clean_san(san) }
...
FILENAME==poolbasefile { pool_monitor[$1]=$2; pool_avail[$1]=$3; pool_enabled[$1]=$4; pool_reason[$1]=$5; pool_extra[$1]=$6; pool_bitsin[$1]=$7; pool_bitsout[$1]=$8; pool_cur[$1]=$9; pool_max[$1]=$10; pool_tot[$1]=$11; next }
...
END { for (vip in vips) { ip=destination_ip(raw_dest[vip]); port=destination_port(raw_dest[vip]); resolved=""; if (ip!="") { if (ip in dns) resolved=dns[ip]; else resolved="NR" } pool=vip_pool[vip]; print vip,ip,port,resolved,avail[vip],enabled[vip],reason[vip],extra[vip],bitsin[vip],bitsout[vip],cur[vip],max[vip],tot[vip],pool,pool_monitor[pool],pool_avail[pool],pool_enabled[pool],pool_reason[pool],pool_extra[pool],pool_ips[pool],pool_ports[pool],pool_admin[pool],pool_mavail[pool],pool_menabled[pool],pool_mreason[pool],pool_bitsin[pool],pool_bitsout[pool],pool_cur[pool],pool_max[pool],pool_tot[pool],pool_resolved[pool],client_profiles[vip],server_profiles[vip],client_certs[vip],server_certs[vip],rules[vip] } }
AWK
...
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    ...
    'destination_resolved' 'availability_state' 'enabled_state' 'status_reason' 'status_extra' \
    'clientside_bits_in' 'clientside_bits_out' 'clientside_cur_conns' 'clientside_max_conns' 'clientside_tot_conns' \
    'pool_name' 'pool_monitor' 'pool_availability_state' 'pool_enabled_state' 'pool_status_reason' \
    'pool_status_extra' 'pool_member_ips' 'pool_member_ports' 'pool_member_admin_states' 'pool_member_availability_states' \
    'pool_member_enabled_states' 'pool_member_status_reasons' 'pool_serverside_bits_in' 'pool_serverside_bits_out' 'pool_serverside_cur_conns' \
    'pool_serverside_max_conns' 'pool_serverside_tot_conns' 'pool_member_resolved' 'client_ssl_profiles' 'server_ssl_profiles' \
    'client_certificates' 'server_certificates' 'irules' > "$VIP_OUT_FILE" || exit 1
