...
#
# Usage:
#   ./collect_rseries_inventory_v4.ksh [hosts_file] [output_file]
#
# Defaults:
...
#
# Example:
#   DEBUG=1 SSH_VERBOSE=1 ./collect_rseries_inventory_v4.ksh hosts.inv debug.tsv
#
# Controls:
...
save_debug_files()
{
    ...
    cp "$1" \
        "$DEBUG_DIR/${PREFIX}.out" \
        2>/dev/null

    cp "$SSH_ERR_TMP" \
        "$DEBUG_DIR/${PREFIX}.ssh.log" \
        2>/dev/null

    cp "$COMMAND_TMP" \
        "$DEBUG_DIR/${PREFIX}.command" \
        2>/dev/null
}
...
do
    ...
    # A non-FIPS host rejects "show fips".
    # This is an expected result and is not a collection failure.
    if grep -i \
    ...
    # Ensure that the first AWK input is non-empty. This keeps the
    # FNR == NR file split reliable on hosts where show fips is unsupported.
    ...
    awk \
        ...
        function rest_of_line(start, value, i) {
            value=""
        ...
        function emit_value(value, first) {
            if (!first) {
                printf "\t"
            }

            printf "\"%s\"", clean(value)
        }

        function row_status(base, extra) {
            if (extra == "") {
                return base
            }
        ...
        function emit_tenant(
            ...
            key=tolower(tenant)
            partition=tenant_partition[key]
            mapped_pci=tenant_pci[key]
            extra_status=""
            ...
            if (
                ...
                extra_status="FIPS_PARTITION_NOT_FOUND"
            }
            ...
            emit_value(partition, 0)
            emit_value(partition_keys[partition], 0)
            emit_value(partition_accel_devs[partition], 0)
            emit_value(partition_backup[partition], 0)
            emit_value(partition_state[partition], 0)
            emit_value(
                partition_occupied_session_keys[partition],
                0
            )
            emit_value(partition_session_count[partition], 0)
            emit_value(partition_pci_address[partition], 0)
            emit_value(mapped_pci, 0)
            emit_value(
                row_status(
                    collect_status,
                    extra_status
        ...
        # First input file: output from "show fips".
        FNR == NR {
            gsub(/\r/, "")
            ...
            if (
                ...
                if ($3=="occupied-acclr-dev") {
                    fips_occupied_acclr_dev=$4
                }

                if ($3=="occupied-contexts") {
                    fips_occupied_contexts=$4
                }

                if ($3=="occupied-keys") {
                    fips_occupied_keys=$4
                }

                if ($3=="occupied-partitions") {
                    fips_occupied_partitions=$4
                }

                if ($3=="total-acclr-dev") {
                    fips_total_acclr_dev=$4
                }

                if ($3=="total-contexts") {
                    fips_total_contexts=$4
                }
                ...
                if ($3=="total-partitions") {
                    fips_total_partitions=$4
                }
            ...
            if (
                ...
                fips_status_desc=rest_of_line(4)
                next
            }

            # FIPS partition resources table:
            #
            # NAME
            # KEYS
            # ACCEL DEVS
            ...
            if (
                ...
                partition_keys[partition]=$2
                partition_accel_devs[partition]=$3
                partition_backup[partition]=$4
                partition_state[partition]=$5

                partition_occupied_session_keys[partition]=$6
                partition_session_count[partition]=$7
                partition_pci_address[partition]=$8

                next
            }

            # Tenant-to-FIPS-partition mapping table:
            #
            # TENANT_NAME
            # PARTITION
            # PCI_DEVICE_ID
        ...
        # Second input file: output from "show tenants".
        BEGIN {
            emitted=0
            clear_tenant_values()
        }
        ...
        $1=="state" &&
        $2=="status" {
            tenant_status=rest_of_line(3)
            next
        }

        $1=="state" &&
        $2=="image-version" {
            image_version=rest_of_line(3)
            next
        }
