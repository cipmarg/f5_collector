...
run_tmsh()
{
    ...
    # The remote login shell is already tmsh.
    # All output processing is performed locally.
    printf 'y\n' |
...
# Extract an exact DN attribute.
# This prevents O= from being confused with OU=.
...
# Parse comma-separated DN attributes while preserving escaped commas.
...
function add_vip_usage(profile, context) {
    ...
    if (context == "clientside") {
        usage[
            "clientssl" SUBSEP profile SUBSEP "VIP_" vip
        ] = 1
    }
    else if (context == "serverside") {
        usage[
            "serverssl" SUBSEP profile SUBSEP "VIP_" vip
        ] = 1
    }
    else {
        usage[
...
function finish_monitor() {
    ...
    if (
        ...
        usage[
            "serverssl" SUBSEP mprofile SUBSEP \
            "HTTPS_MON_" mname
        ] = 1
    }
}
...
section == "CERTS" {
    ...
    if (depth > 0) {
        ...
        if (text ~ /^expiration-date[[:space:]]+/) {
            ...
            text ~ /^expiration-string[[:space:]]+/
        ) {
            value = text

            sub(
                /^expiration-string[[:space:]]+/,
                "",
                value
            )
...
section == "SERVERSSL" {
    if (
        $0 ~ /^ltm profile (client-ssl|server-ssl)[[:space:]]+/
    ) {
        if (section == "CLIENTSSL") {
            ptype = "clientssl"
        }
        else {
            ptype = "serverssl"
...
END {
    for (u in usage) {
        split(u, usage_fields, SUBSEP)

        usage_type = usage_fields[1]
        usage_profile = usage_fields[2]
        used_by = usage_fields[3]

        for (p in profile_pair) {
            split(p, profile_fields, SUBSEP)

            if (
                profile_fields[1] != usage_type ||
                profile_fields[2] != usage_profile
            ) {
                continue
            }

            cert = profile_fields[3]
            key = profile_fields[4]

            if (!(cert in cert_ok)) {
                continue
            }

            if (key in key_security) {
            ...
            rowkey = \
                cert SUBSEP \
                usage_profile SUBSEP \
                used_by SUBSEP \
                cert_exp[cert] SUBSEP \
                security SUBSEP \
                cert_org[cert] SUBSEP \
                cert_cn[cert] SUBSEP \
                cert_issuer_cn[cert]
...
' "$hostdir/all.raw" |
        LC_ALL=C sort -u > "$hostdir/result.tsv"
    ...
    if [ "$host_rows" -gt 0 ]; then
        ...
        echo \
            "[$host] Exported $host_rows certificate usage row(s)." \
            >&2
    else
        echo \
            "[$host] No matching certificates found." \
            >&2
    fi
...
echo \
    "Completed: $total_rows row(s) written to $OUT_FILE; " \
    "$failed_hosts host(s) failed." \
    >&2
